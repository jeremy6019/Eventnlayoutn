 ...
 ** GUI(Graphic User Interface- 아이콘이나 메) Programming <-> CLI(Command Line Interface)- 명령어 입력방식 
   1.AWT - 운영체제의 자원을 빌려서 GUI를 표형 
  => 자원의 다양성에 한계까 있고 시스템이 느려지고 운영체제 별로 다르게 보입니다. 
   ...
   ** 용어 
    ...
    ** AWT
    =>윈도우를 만들어서 컴포넌트들을 배치 
    1. Layout:배치방법 
    1) FlowLayout - Panel의 기본 배치방법으로 옆으로 계쏙해서 배치하고 공간이 없으면 아래로 배치 
   ...
   ** 이벤트 처리 
   ...
   => 버튼을 누른다던가 위도우가 종료된다던가 하는 것이 전부 이벤트 
   ...
   1) 컴포넌트 자체가 가진 이벤트처리 메소드를 오버라이딩: 스마트폰 API에 존재하고 PC용 윈도우 프로그래밍에서는 드로윙 
    ...
    => AWT, Swing, f/c, Android, IOs,web등 거의 모든 프로그래밍 분야에서 사용하는 방식 
   ...
   편리하게 하기 위해서 입니다. 
   컴포넌트가 존재하는 경우 이컴포넌트가 보여지는 설정과 이벤트처리를 모두 한다면 보여지는 부분만 수정하고자 할때 
...
=>ItemEvent에는 getItem()이라는 메소드로 이벤트를 발생시킨 항목을 리턴해죽ㅎ getStateChange()메소드로 
